...
export function normalizeRole(role: string | undefined | null): CanonicalRole | 'PENDING' | 'USER' {
  ...
  /** Catha DB uses ADMIN for bar managers (see CathaUserRole). */
...
/**
 * Whether a sidebar link should show for DB-derived allowed routes.
 * `/catha` is dashboard only — never treat it as a prefix for all `/catha/*` paths (that was showing every nav item).
 */
export function navItemHrefAllowed(itemHref: string, allowedRoutes: string[]): boolean {
  const h = (itemHref || '').replace(/\/$/, '') || itemHref
  return allowedRoutes.some((raw) => {
    const r = (raw || '').replace(/\/$/, '') || raw
    if (!r) return false
    if (h === r) return true
    if (r === '/catha') return false
