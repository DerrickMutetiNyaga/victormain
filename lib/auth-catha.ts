...
/** Load role/status/permissions from DB — never trust JWT alone for API authorization. */
async function getCathaAccessFromDb(email: string): Promise<
  | { ok: true; role: CathaUserRole; permissions: CathaPermissions }
  | { ok: false; response: NextResponse }
> {
  const cu = await getCathaUserByEmail(email)
  if (!cu) {
    return { ok: false, response: NextResponse.json({ error: 'User not found' }, { status: 404 }) }
  }
  const role = normalizeCathaRole(cu.role as unknown as string)
  const statusUpper = (String(cu.status ?? 'PENDING')).toUpperCase()
  const permissions: CathaPermissions = normalizePermissions(cu.permissions)

  if (statusUpper === 'PENDING') {
    return { ok: false, response: NextResponse.json({ error: 'Account pending approval' }, { status: 403 }) }
  }
  if (statusUpper === 'DISABLED' || statusUpper === 'SUSPENDED') {
...
export async function requireCathaPermission(permissionKey: string, action: LegacyAction = 'view') {
  ...
  const access = await getCathaAccessFromDb(session.user.email)
  if (!access.ok) return { allowed: false, response: access.response }
  ...
  const ok = hasCathaPermission(access.permissions, mapping.module, mapping.action)
...
/** Allows unauthenticated callers; when authenticated, enforces at least one permission entry (DB-backed). */
export async function requireCathaPermissionOrPublic(entries: LegacyPermissionInput[]) {
  ...
  const ok = entries.some((entry) => {
    ...
    return mapping ? hasCathaPermission(access.permissions, mapping.module, mapping.action) : false
  })
...
/** Authenticated-only helper: requires at least one matching permission entry (DB-backed). */
...
/** SUPER_ADMIN guard used in older API routes (role verified from DB). */
export async function requireSuperAdminCatha() {
  ...
  const access = await getCathaAccessFromDb(session.user.email)
  if (!access.ok) {
    return { session: null, response: access.response }
  }
  if (access.role !== 'SUPER_ADMIN') {
