...
import { navItemHrefAllowed, type CathaAccessUser } from "@/lib/catha-access"
import { CATHA_AUTH_ME_REFRESH_EVENT } from "@/lib/catha-permissions-model"
import { CATHA_NAV_ITEMS, CATHA_NAV_SECTIONS, CATHA_SUPER_ADMIN_NAV_ITEMS } from "@/lib/catha-navigation"
...
function formatSidebarRoleLabel(raw: string | undefined | null): string {
  if (!raw) return "User"
...
export function Sidebar() {
  ...
  const fetchCathaNavUser = useCallback(() => {
  ...
  /** DB-backed role/routes on every Catha navigation and reload (visibility + events also call fetchCathaNavUser). */
  useEffect(() => {
    if (!isCatha) return
    fetchCathaNavUser()
  }, [isCatha, pathname, fetchCathaNavUser])

  useEffect(() => {
    if (!isCatha) return
    const onRefresh = () => fetchCathaNavUser()
    window.addEventListener(CATHA_AUTH_ME_REFRESH_EVENT, onRefresh)
    return () => window.removeEventListener(CATHA_AUTH_ME_REFRESH_EVENT, onRefresh)
  }, [isCatha, fetchCathaNavUser])

  useRefetchOnVisibility(isCatha, fetchCathaNavUser)

  useEffect(() => {
    if (!isCatha || !authUser) return
    const isSA = (authUser.role ?? '').toUpperCase() === 'SUPER_ADMIN'
    if (!isSA) return
    fetch('/api/catha/ai-intelligence/alerts-count', { cache: 'no-store' })
      .then(r => r.json())
      .then(d => { if (typeof d?.count === 'number') setAiAlertCount(d.count) })
      .catch(() => {})
  }, [isCatha, authUser])
  ...
  const filteredNavigation = useMemo(() => {
    ...
    return CATHA_NAV_ITEMS.filter((item) => navItemHrefAllowed(item.href, routes))
  }, [authUser, isCatha])
  ...
  const filteredNavSections = useMemo(() => {
    ...
    return CATHA_NAV_SECTIONS.map((s) => ({
      title: s.title,
      items: s.items.filter((item) => navItemHrefAllowed(item.href, routes)),
  ...
  const sidebarRoleLabel = useMemo(() => {
    if (isCatha && authUser?.role) return formatSidebarRoleLabel(authUser.role)
    return formatSidebarRoleLabel(session?.user?.role as string | undefined)
  }, [isCatha, authUser?.role, session?.user?.role])
        ...
        <nav className="flex-1 overflow-y-auto">
          ...
          <div className="hidden xl:block space-y-1 px-3 py-4 text-sm">
            ...
            {isCatha && authUser === undefined ? (
              ...
              {/* Super Admin Only: AI Intelligence */}
              {(authUser?.role ?? '').toUpperCase() === 'SUPER_ADMIN' && (
                <>
                  <div className="my-2 mx-3 border-t border-sidebar-border/40" />
                  <p className="mb-2 px-3 text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground/80">Intelligence</p>
                  {CATHA_SUPER_ADMIN_NAV_ITEMS.map((item) => {
                    ...
                    return (
                      <Link key={item.name} href={item.href} className={cn(
                        ...
                        <span className={cn("h-7 w-1 rounded-full bg-transparent transition-colors", isActive ? "bg-sidebar-primary-foreground/80" : "group-hover:bg-sidebar-accent-foreground/40")} />
                        <item.icon className={cn("h-5 w-5 transition-colors", isActive ? "text-indigo-300" : "text-indigo-500 group-hover:text-indigo-400")} />
          ...
          <div className="xl:hidden py-4 px-2">
            {isCatha && authUser === undefined ? (
              ...
              {/* Super Admin Only: AI Intelligence (mobile) */}
              {(authUser?.role ?? '').toUpperCase() === 'SUPER_ADMIN' && (
                <div className="mb-6">
                  <p className="mb-2 px-3 text-[10px] font-semibold uppercase tracking-widest text-[#94a3b8]">Intelligence</p>
                  <div className="space-y-0.5">
                    {CATHA_SUPER_ADMIN_NAV_ITEMS.map((item) => {
                      const isActive = pathname === item.href || pathname?.startsWith(item.href + '/')
                      return (
                        <Link key={item.name} href={item.href} onClick={() => isDrawerMode && setIsOpen(false)}
                          className={cn(
                            "group flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all",
                            isActive ? "bg-[#ecfdf5] text-[#059669] border-l-[3px] border-l-primary pl-[9px]" : "text-[#475569] hover:bg-[#f8fafc] hover:text-[#0f172a]"
                          )}>
                          <item.icon className={cn("h-4 w-4 shrink-0", isActive ? "text-primary" : "text-indigo-500 group-hover:text-indigo-400")} />
                          <span className="flex-1 truncate">{item.name}</span>
                          {aiAlertCount > 0 && (
                            <span className="flex h-5 min-w-[20px] items-center justify-center rounded-full bg-red-500 px-1.5 text-[10px] font-bold text-white animate-pulse">
                              {aiAlertCount}
            ...
            <DropdownMenuTrigger asChild>
              <button className="flex w-full items-center gap-3 rounded-xl p-2 transition-colors hover:bg-sidebar-accent">
                ...
                <div className="flex-1 text-left">
                  ...
                  <p className="text-xs text-muted-foreground">{sidebarRoleLabel}</p>
                </div>
              </button>
            </DropdownMenuTrigger>
        ...
        <div className="xl:hidden border-t border-sidebar-border/80 p-4 bg-[#f8fafc]/50 shrink-0">
          <div className="flex items-center gap-3 rounded-xl p-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-[#64748b] truncate">{sidebarRoleLabel}</p>
            </div>
          </div>
