...
export function AddStockModal({ product, onSuccess }: AddStockModalProps) {
  ...
  const [categoryOptions, setCategoryOptions] = useState<BarCategoryOption[]>([])
  const [volumeOptions, setVolumeOptions] = useState<VolumeOption[]>([])
  const [newCategoryName, setNewCategoryName] = useState("")
  const [newVolumeAmount, setNewVolumeAmount] = useState("")
  const [newVolumeUnit, setNewVolumeUnit] = useState<"ml" | "L" | "g" | "kg">("ml")
  ...
  // Load categories & volumes when the dialog first opens
  useEffect(() => {
    const shouldLoad = isEditMode ? !!product : open
    if (!shouldLoad) return

    const loadLookups = async () => {
      try {
        const [catRes, volRes] = await Promise.all([
          fetch('/api/catha/bar-categories'),
          fetch('/api/catha/bar-volumes'),
        ])
        ...
        console.error('Failed to load bar lookups:', err)
      }
    }
        ...
        <div className="space-y-6 py-6">
            ...
            <div className="space-y-2">
                  ...
                  <div className="mt-2 flex gap-2">
                    <Input
                      ...
                      className="rounded-lg border-2 focus:border-amber-500 h-10"
                    />
                    <Button
                      type="button"
                      variant="outline"
                      ...
                      onClick={async () => {
                        ...
                        if (!name) return
                        try {
                          const response = await fetch('/api/catha/bar-categories', {
                            method: 'POST',
                          ...
                          const data = await response.json()
                          const created = data.category as BarCategoryOption & { id: string }
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2 md:col-span-3">
                  ...
                  <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50/70 p-4 shadow-sm">
                    {/* Row 1: Preset size (primary) */}
                    <Select
                      value={formData.size}
                      onValueChange={(v) => {
                        setFormData({ ...formData, size: v })
                        // Preset OR custom. Selecting preset clears custom inputs.
                        setNewVolumeAmount("")
                        setNewVolumeUnit("ml")
                      }}
                    >
                      <SelectTrigger className="h-11 w-full rounded-xl border border-slate-200 bg-white focus:ring-blue-200">
                        <SelectValue placeholder="Select preset size" />
                      </SelectTrigger>
                      <SelectContent>
                        {volumeOptions.map((vol) => (
                          <SelectItem key={vol.id} value={vol.value}>
                            {vol.value}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    ...
                    <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center">
                      <Input
                        ...
                        value={newVolumeAmount}
                        onChange={(e) => {
                          const v = e.target.value
                          setNewVolumeAmount(v)
                          // Preset OR custom. Typing custom clears preset selection.
                          if (v.trim().length > 0) {
                            setFormData((prev) => ({ ...prev, size: "" }))
                          }
                        }}
                        className="h-11 w-full rounded-xl border border-slate-200 bg-white focus-visible:ring-blue-200 sm:w-[110px]"
                      />

                      <Select
                        value={newVolumeUnit}
                        onValueChange={(v) => setNewVolumeUnit(v as "ml" | "L" | "g" | "kg")}
                      >
                        <SelectTrigger className="h-11 w-full rounded-xl border border-slate-200 bg-white focus:ring-blue-200 sm:w-[90px]">
                      ...
                      <Button
                        type="button"
                        className="h-11 w-full rounded-xl bg-blue-600 text-white hover:bg-blue-700 sm:w-[140px]"
                        disabled={!newVolumeAmount.trim()}
                        onClick={async () => {
                          const amount = newVolumeAmount.trim()
                          if (!amount) return

                          const unit = (newVolumeUnit || "ml") as string
                          const value = `${amount}${unit}`

                          try {
                            const response = await fetch('/api/catha/bar-volumes', {
                              method: 'POST',
                              headers: { 'Content-Type': 'application/json' },
                            ...
                            if (!response.ok) {
                              const error = await response.json().catch(() => ({}))
                              throw new Error(error.error || 'Failed to add size')
                            }
                            const data = await response.json()
                            const created = data.volume as VolumeOption
                            setVolumeOptions((prev) => [...prev, created])
                            setFormData((prev) => ({ ...prev, size: created.value }))
                            setNewVolumeAmount("")
                            setNewVolumeUnit("ml")
                    ...
                    <div className="mt-3 text-xs text-muted-foreground sm:whitespace-nowrap">
          ...
          <Card className="border-2 border-emerald-200/50 bg-gradient-to-br from-emerald-50/50 to-teal-50/30">
            ...
            <div>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl bg-white/60 border-2 border-amber-300/50">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-lg bg-gradient-to-br from-amber-500 to-yellow-500 flex items-center justify-center shadow-md">
                    <Sparkles className="h-5 w-5 text-white" />
                  </div>
                  <div>
                    <Label htmlFor="featured" className="text-base font-bold text-foreground cursor-pointer">
                      Featured Product
                    </Label>
                    <p className="text-sm text-muted-foreground mt-0.5">
                      Show on homepage Featured Products (maximum 8 displayed)
                    </p>
                  </div>
                </div>
