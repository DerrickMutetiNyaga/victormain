...
export function PromotionBundlesTab({ campaigns = [] }: { campaigns?: CampaignRow[] }) {
  ...
  const save = async () => {
    const productIds = form.productIds.split(/[\s,]+/).map((s) => s.trim()).filter(Boolean)
    if (productIds.length < 2) {
      toast.error("Enter at least 2 product IDs")
      return
    }
    setSaving(true)
    try {
  ...
  return (
    <div className="px-4 py-4 sm:p-5 space-y-5">
      <section className="rounded-lg border p-4 space-y-4">
        ...
        <p className="text-xs text-muted-foreground">Fixed price when all listed products are in the cart (one unit each per bundle set).</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Input type="number" value={form.bundlePrice} onChange={(e) => setForm((f) => ({ ...f, bundlePrice: e.target.value }))} className="mt-1 h-10" />
          </div>
          <div>
          ...
          <div className="sm:col-span-2">
            <Label className="text-xs text-muted-foreground">Product IDs (comma or space separated)</Label>
            <Input value={form.productIds} onChange={(e) => setForm((f) => ({ ...f, productIds: e.target.value }))} className="mt-1 h-10 font-mono text-xs" placeholder="id1, id2" />
          </div>
          <div>
