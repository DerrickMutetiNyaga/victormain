...
type ReportData = {
  ...
  profitSummary: {
    weekly: { revenue: number; cost: number; profit: number }
    monthly: { revenue: number; cost: number; profit: number }
    yearly: { revenue: number; cost: number; profit: number }
  }
...
export default function ReportsPage() {
  ...
  const [appliedStartDate, setAppliedStartDate] = useState<string>(isoDate(new Date(now.getFullYear(), now.getMonth(), 1)))
  const [appliedEndDate, setAppliedEndDate] = useState<string>(isoDate(now))
  const [loading, setLoading] = useState(true)
  const [data, setData] = useState<ReportData>({
    summary: { revenue: 0, orders: 0, avgOrderValue: 0 },
    profitSummary: {
      weekly: { revenue: 0, cost: 0, profit: 0 },
      monthly: { revenue: 0, cost: 0, profit: 0 },
      yearly: { revenue: 0, cost: 0, profit: 0 },
    },
    paymentBreakdown: [],
    topProducts: [],
  })

  const loadReports = useCallback(async (rangeStart: string, rangeEnd: string) => {
  ...
  const rangeLabel = `${appliedStartDate} to ${appliedEndDate}`
      ...
      <div className="space-y-6 p-4 sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
            <div className="flex w-full flex-col gap-2 rounded-md border px-3 py-2 sm:w-auto sm:flex-row sm:items-center sm:gap-2 sm:px-2 sm:py-1.5">
              ...
              <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
                <Input
                  ...
                  className="h-8 w-full border-0 p-0 shadow-none focus-visible:ring-0 sm:w-[145px]"
                />
            ...
            <Button variant="outline" className="w-full gap-2 bg-transparent sm:w-auto" onClick={handleApplyDateRange} disabled={loading}>
              <Calendar className="h-4 w-4" />
              Apply Range
            </Button>
            <Button variant="outline" className="w-full gap-2 bg-transparent sm:w-auto" onClick={handleExportReport}>
        ...
        <Card className="border-border bg-card">
          <CardContent className="p-5 space-y-3">
            <h3 className="text-sm font-semibold text-foreground">Profit summary (Buying vs selling)</h3>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <div className="rounded-lg border border-border p-3">
                <p className="text-xs text-muted-foreground">Weekly Profit</p>
                <p className="mt-1 text-lg font-semibold text-foreground">{formatKsh(Math.round(data.profitSummary.weekly.profit || 0))}</p>
              </div>
              <div className="rounded-lg border border-border p-3">
                <p className="text-xs text-muted-foreground">Monthly Profit</p>
                <p className="mt-1 text-lg font-semibold text-foreground">{formatKsh(Math.round(data.profitSummary.monthly.profit || 0))}</p>
              </div>
              <div className="rounded-lg border border-border p-3">
                <p className="text-xs text-muted-foreground">Yearly Profit</p>
                <p className="mt-1 text-lg font-semibold text-foreground">{formatKsh(Math.round(data.profitSummary.yearly.profit || 0))}</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            {data.paymentBreakdown.length === 0 ? (
              ...
              <>
                ...
                <div className="space-y-2 md:hidden">
                  {data.paymentBreakdown.map((row) => (
                    <div key={row.method} className="rounded-lg border border-border p-3">
                      <p className="text-sm font-medium capitalize text-foreground">{row.method || "Unknown"}</p>
                      <p className="mt-1 text-xs text-muted-foreground">Orders: {row.count}</p>
                      <p className="text-sm font-semibold text-foreground">{formatKsh(Math.round(row.total || 0))}</p>
                    </div>
                  ))}
                </div>
              </>
          ...
          <CardContent className="p-5 space-y-3">
            ...
            {data.topProducts.length === 0 ? (
              ...
              <>
                ...
                <div className="space-y-2 md:hidden">
                  {data.topProducts.map((row, index) => (
                    <div key={row.productId || index} className="rounded-lg border border-border p-3">
                      <p className="text-xs text-muted-foreground">#{index + 1}</p>
                      <p className="text-sm font-medium text-foreground">{row.name || row.productId}</p>
                      <p className="mt-1 text-xs text-muted-foreground">Units: {row.quantity}</p>
                      <p className="text-sm font-semibold text-foreground">{formatKsh(Math.round(row.revenue || 0))}</p>
                    </div>
                  ))}
                </div>
              </>
