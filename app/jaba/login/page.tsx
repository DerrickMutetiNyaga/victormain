...
import { safeJabaCallbackUrl } from "@/lib/auth-callback-url"
...
function LoginPageContent() {
  ...
  /** Server-only httpOnly cookie via API — do not set auth_context from JS (spoofable). */
  const postLoginPath = safeJabaCallbackUrl(searchParams.get("callbackUrl"))
  const error = searchParams.get("error")
  const hasRedirected = useRef(false)

  useEffect(() => {
    fetch("/api/auth/jaba-setup", { credentials: "same-origin" }).catch(console.error)
  }, [])
  ...
  /** After OAuth, only trust JWT/session for app identity — route access is enforced in middleware (signed token), not client permission heuristics. */
  useEffect(() => {
    if (hasRedirected.current) return
    if (error) return
    if (status === "loading" && !sessionLoadTimedOut) return
  ...
  }, [session, status, sessionLoadTimedOut, postLoginPath, router, error])
  ...
  const isJabaSession = Boolean(session?.user && userCollection === "jaba")
