...
const navigationGroups = [
  {
    name: "Overview",
    icon: Home,
    items: [{ name: "Dashboard", href: "/jaba", icon: LayoutDashboard, pageId: "dashboard" }],
  },
  {
    name: "Production",
    icon: Cog,
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Material flow", href: "/jaba/raw-materials/flow", icon: ArrowDownUp, pageId: "raw-materials" },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Distributor Requests", href: "/jaba/distributor-requests", icon: FileText, pageId: "distributor-requests" },
    ],
  },
  {
    name: "Analytics",
...
/** Sidebar active state: `/jaba/raw-materials` is not active on `/jaba/raw-materials/flow` (sibling page). */
function isJabaNavItemActive(pathname: string | null, href: string): boolean {
  if (!pathname) return false
  if (pathname === href) return true
  if (href === "/jaba") return false
  if (href === "/jaba/raw-materials") {
    if (!pathname.startsWith("/jaba/raw-materials/")) return false
    if (pathname.startsWith("/jaba/raw-materials/flow")) return false
    return true
  }
...
function JabaLayoutContent({
  ...
  const shouldRefetchJabaMe =
    Boolean(pathname?.startsWith("/jaba")) &&
    !isJabaPublicAuthPath &&
  ...
  const loadJabaMe = useCallback(() => {
    if (status !== "authenticated" || !session?.user?.email) return
    fetch("/api/jaba/auth/me", { cache: "no-store" })
  ...
  /** DB-backed role/permissions on every protected Jaba navigation, reload, and tab return. */
  useEffect(() => {
    if (!shouldRefetchJabaMe) return
    loadJabaMe()
  }, [shouldRefetchJabaMe, loadJabaMe, pathname])

  useRefetchOnVisibility(shouldRefetchJabaMe, loadJabaMe)

  useEffect(() => {
    if (userForPermissions?.role?.toLowerCase() !== 'super_admin') return
    fetch('/api/jaba/ai-intelligence/alerts-count', { cache: 'no-store' })
      .then((r) => r.json())
      .then((d) => {
        if (typeof d?.count === 'number') setAiAlertCount(d.count)
      })
      .catch(() => {})
  ...
  useEffect(() => {
    const checkPermissions = async () => {
      ...
      try {
        const response = await fetch('/api/jaba/auth/me', { cache: 'no-store' })
        if (!response.ok) {
          router.push('/jaba/waiting')
          return
        }
  ...
  useEffect(() => {
    if (hasAccess) {
      ...
      const initialOpenGroups = navigationGroups
        .filter((group) => group.items.some((item) => isJabaNavItemActive(pathname, item.href)))
  ...
  const jabaSidebarRoleLabel = useMemo(() => {
    const r = (meData?.role ?? session?.user?.role ?? "").toLowerCase()
    if (!r) return "User"
    if (r === "super_admin") return "Super Admin"
    if (r === "cashier_admin") return "Cashier Admin"
    if (r === "manager_admin") return "Manager Admin"
    if (r === "pending") return "Pending"
    return r.charAt(0).toUpperCase() + r.slice(1)
  }, [meData?.role, session?.user?.role])
    ...
    <div className="flex min-h-screen bg-[#F1F5F9] dark:bg-slate-900 overflow-hidden">
      ...
      <aside className={cn(
        ...
        <div className="flex h-full flex-col overflow-hidden overflow-x-hidden">
          ...
          {/* Navigation: scrollable groups + INTELLIGENCE pinned at bottom (super admin) */}
          <nav className="flex-1 flex flex-col min-h-0 overflow-hidden">
            ...
            {filteredNavigationGroups.map((group, index) => {
              ...
              const hasActiveItem = group.items.some((item) => isJabaNavItemActive(pathname, item.href))
                      ...
                      <CollapsibleContent className="mt-1">
                        <div className={cn("space-y-2", isMobile ? "pl-0" : "pl-1")}>
                          {group.items.map((item) => {
                            const isActive = isJabaNavItemActive(pathname, item.href)
                            return (
                              <Link
                                key={item.name}
                                href={item.href}
            ...
            {/* INTELLIGENCE — own section, last in sidebar (super admin only); matches Catha / design reference */}
            {userForPermissions?.role?.toLowerCase() === 'super_admin' && (
              <div
                className={cn(
                  'shrink-0 border-t px-4 pt-3 pb-2',
                  isMobile
                    ? 'border-emerald-200/40 dark:border-slate-800/60 bg-gradient-to-b from-emerald-50/90 to-emerald-50/70 dark:bg-slate-900'
                    : 'border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-[#0F172A]'
                )}
              >
                <p className="mb-2 px-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-[#94a3b8]">
                  Intelligence
                </p>
                <Link
                  href="/jaba/ai-intelligence"
                  onClick={() => isMobile && setSidebarOpen(false)}
                  className={cn(
                    'group relative flex w-full items-center gap-3 rounded-xl px-3 py-2.5 transition-all duration-150',
                    isJabaNavItemActive(pathname, '/jaba/ai-intelligence')
                      ? isMobile
                        ? 'bg-gradient-to-r from-emerald-600 to-emerald-700 text-white shadow-lg'
                        : 'bg-gradient-to-r from-emerald-600 to-emerald-700 text-white shadow-md'
                      : isMobile
                        ? 'text-[#334155] dark:text-slate-200 hover:bg-white/60 dark:hover:bg-slate-800/40'
                        : 'text-[#334155] dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/50'
                  )}
                >
                  <Brain
                    className={cn(
                  ...
                  <span
                    className={cn(
                      ...
                      isJabaNavItemActive(pathname, '/jaba/ai-intelligence') ? 'text-white' : ''
                    )}
                  >
                    AI Intelligence
                  </span>
                  {aiAlertCount > 0 && (
                    <span className="flex h-5 min-w-[22px] shrink-0 items-center justify-center rounded-full bg-[#f87171] px-1.5 text-[10px] font-bold text-white shadow-sm">
                      {aiAlertCount}
                    </span>
                  )}
                </Link>
