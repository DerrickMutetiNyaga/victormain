...
/** Stable list id for expand state (always string). */
function batchListId(batch: { _id?: string; id?: string }): string {
  return String(batch._id ?? batch.id ?? "")
}

/** Litres to show for a flavour row (new lines use allocatedLitres; legacy uses infused/total). */
function flavourOutputAllocDisplayLitres(k: FlavourOutputRow): string {
  const n = Number(k.allocatedLitres ?? k.infusedQuantityLitres ?? k.totalLitres ?? 0)
  return `${Number.isFinite(n) ? n.toFixed(2) : "0.00"}L`
}

function flavourOutputRowKey(k: FlavourOutputRow, index: number): string {
  const id = k._id ?? k.id
  return id != null && String(id) !== "" ? String(id) : `flavour-row-${index}`
}

/** Legacy child batches have their own batch document; flavour-line rows live under the parent batch page. */
function flavourOutputViewHref(k: FlavourOutputRow, parentBatchId: string): string {
  if (k.isLegacyChildBatch) {
    return `/jaba/batches/${encodeURIComponent(String(k._id ?? k.id ?? ""))}`
  }
  return `/jaba/batches/${encodeURIComponent(parentBatchId)}`
}
...
export default function BatchesPage() {
  ...
  const availableNeutralLitres =
    selectedInfuseParent?.neutralRemainingLitres ??
    selectedInfuseParent?.outputSummary?.remainingLitres ??
    Math.max(0, (selectedInfuseParent?.totalLitres || 0) - (selectedInfuseParent?.infusedAllocatedLitres || 0))
  const plannedInfuseLitres = infuseRows.reduce((sum, row) => sum + (Number(row.quantity) || 0), 0)
  const infuseLitresLeft = Math.max(0, availableNeutralLitres - plannedInfuseLitres)
  const infuseOverAllocated = plannedInfuseLitres > availableNeutralLitres
  ...
  return (
    <>
                          ...
                          <div className="flex items-center justify-end gap-1.5 flex-wrap">
                            ...
                            {isSuperAdmin && (
                              <Button
                                ...
                                className="h-8 w-8 p-0 border-red-300 text-red-600 hover:bg-red-50 dark:border-red-900 dark:text-red-400 dark:hover:bg-red-950/30 rounded-lg"
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {isSuperAdmin && (
                        <Button
                          ...
                          size="sm"
                          onClick={() => deleteBatch(batch)}
                          className="flex-1 h-9 border-red-300 text-red-600 hover:bg-red-50 dark:border-red-900 dark:text-red-400 dark:hover:bg-red-950/30 rounded-lg text-xs font-medium"
                        >
                      ...
                      <div className="border-t border-slate-200 dark:border-slate-800 pt-3 space-y-2">
                        ...
                        {expanded && (
                          <ul className="space-y-2 text-xs border rounded-lg border-slate-200 dark:border-slate-700 p-2 bg-slate-50/80 dark:bg-slate-900/50">
                            {kids.filter(Boolean).map((k, idx) => {
                              const fk = k as FlavourOutputRow
                              const viewHref = flavourOutputViewHref(fk, bid)
                              return (
                              <li
                                key={flavourOutputRowKey(fk, idx)}
                              ...
                              >
                                <div>
                                  <span className="font-mono font-medium">{fk.lineCode || fk.batchNumber || "—"}</span>
                                  <span className="text-slate-600 dark:text-slate-400 ml-2">{fk.flavourName || fk.flavor || "—"}</span>
                                  <span className="block text-slate-500">
                                    {flavourOutputAllocDisplayLitres(fk)}
                                  </span>
                                </div>
                                <div className="flex gap-1 shrink-0">
                                  ...
                                  {canEdit && (
                                    <Button
                                      ...
                                      onClick={() => deleteFlavourOutput(String(fk._id || fk.id), batch.batchNumber)}
                                    >
                                      Remove
                                    </Button>
                                  )}
      ...
      <Dialog open={infuseOpen} onOpenChange={setInfuseOpen}>
        <DialogContent className="max-h-[90vh] w-[calc(100vw-1.5rem)] max-w-2xl overflow-y-auto p-0">
          <DialogHeader>
            <div className="border-b border-slate-200 dark:border-slate-800 px-5 py-4 sm:px-6">
          ...
          <div className="space-y-4 px-5 py-4 sm:px-6">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div className="rounded-lg border border-violet-200 bg-violet-50/60 p-3">
                <p className="text-xs font-semibold uppercase tracking-wide text-violet-700">Available</p>
                <p className="mt-1 text-xl font-bold text-violet-900">{availableNeutralLitres.toFixed(2)}L</p>
              </div>
              <div className="rounded-lg border border-cyan-200 bg-cyan-50/60 p-3">
                <p className="text-xs font-semibold uppercase tracking-wide text-cyan-700">Planned</p>
                <p className="mt-1 text-xl font-bold text-cyan-900">{plannedInfuseLitres.toFixed(2)}L</p>
              </div>
              <div
                className={cn(
                  "rounded-lg border p-3",
                  infuseOverAllocated ? "border-red-200 bg-red-50/60" : "border-emerald-200 bg-emerald-50/60",
                )}
              >
                <p className={cn("text-xs font-semibold uppercase tracking-wide", infuseOverAllocated ? "text-red-700" : "text-emerald-700")}>
                  Remaining
                </p>
                <p className={cn("mt-1 text-xl font-bold", infuseOverAllocated ? "text-red-900" : "text-emerald-900")}>
                  {infuseOverAllocated ? `-${(plannedInfuseLitres - availableNeutralLitres).toFixed(2)}L` : `${infuseLitresLeft.toFixed(2)}L`}
                </p>
              </div>
            </div>
            ...
            <div className="h-2 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-800">
              <div
                className={cn(
                  ...
                  infuseOverAllocated ? "bg-red-500" : "bg-violet-600",
                )}
                style={{ width: `${Math.min(100, availableNeutralLitres > 0 ? (plannedInfuseLitres / availableNeutralLitres) * 100 : 0)}%` }}
              />
            </div>

            {infuseOverAllocated && (
              <p className="text-sm font-medium text-red-600">
                Planned litres exceed available neutral stock. Reduce one or more line quantities before creating outputs.
              </p>
            )}
          ...
          <div className="space-y-4 px-5 pb-4 sm:px-6">
            {infuseRows.map((row, i) => (
              <div
                key={i}
                className="rounded-xl border border-violet-200 bg-gradient-to-b from-violet-50/70 to-white p-3 shadow-sm dark:border-violet-900/50 dark:from-violet-950/20 dark:to-slate-950 space-y-3"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <Label className="text-xs font-semibold uppercase tracking-wide text-violet-800 dark:text-violet-300">
                      Flavour line
                    </Label>
                  </div>
                  {infuseRows.length > 1 && (
                ...
                <div className="space-y-1.5">
                  <Label className="text-xs text-slate-700 dark:text-slate-200">Flavour</Label>
                  <Select
                  ...
                  >
                    <SelectTrigger className="h-10 border-slate-200 dark:border-slate-800 focus:border-violet-500 dark:focus:border-violet-400 focus:ring-violet-200 dark:focus:ring-violet-400/30">
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  <div>
                    <Label className="text-xs text-slate-700 dark:text-slate-200">Litres</Label>
                    <div className="relative">
                      <Input
                        ...
                        step="0.01"
                        placeholder="0.00"
                        className="h-10 pr-10 text-right border-slate-200 dark:border-slate-800 focus:border-violet-500 dark:focus:border-violet-400 focus:ring-violet-200 dark:focus:ring-violet-400/30"
                        value={row.quantity}
                        onChange={(e) => {
                          const next = [...infuseRows]
                  ...
                  <div>
                    <Label className="text-xs text-slate-700 dark:text-slate-200">Notes (optional)</Label>
                    <Textarea
                      className="h-10 min-h-[40px] resize-none border-slate-200 dark:border-slate-800 focus:border-violet-500 dark:focus:border-violet-400 focus:ring-violet-200 dark:focus:ring-violet-400/30"
          ...
          <div className="flex flex-col-reverse gap-2 border-t border-slate-200 px-5 py-4 dark:border-slate-800 sm:flex-row sm:justify-end sm:px-6">
            <Button type="button" variant="outline" className="w-full sm:w-auto" onClick={() => setInfuseOpen(false)}>
              Cancel
            </Button>
            <Button
              type="button"
              className="w-full bg-violet-600 hover:bg-violet-700 text-white sm:w-auto"
              disabled={infuseSaving || infuseOverAllocated}
      ...
      >
        <DialogContent className="w-[calc(100vw-1.5rem)] max-w-md max-h-[min(90vh,32rem)] overflow-y-auto rounded-2xl">
          <DialogHeader>
          ...
          <div className="space-y-3">
            ...
            />
            {!deleteOtp.trim() && (
              <p className="text-xs text-muted-foreground">Enter the OTP above to enable the red submit button.</p>
            )}
          </div>
