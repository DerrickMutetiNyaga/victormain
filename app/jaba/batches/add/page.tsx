...
export default function AddBatchPage() {
  ...
  const [startNumberYear, setStartNumberYear] = useState<number>(new Date().getFullYear())
  const [startSequenceInput, setStartSequenceInput] = useState("1")

  const requestNextBatchNumber = async (startSequence?: number) => {
    const url = startSequence
      ? `/api/jaba/batches?nextNumber=true&startSequence=${startSequence}`
      : "/api/jaba/batches?nextNumber=true"
    const response = await fetch(url)
    if (!response.ok) throw new Error('Failed to fetch next batch number')
  ...
  useEffect(() => {
    const fetchNextBatchNumber = async () => {
      try {
        ...
        const data = await requestNextBatchNumber()
        // If the year has no batches yet, ask admin to choose the starting sequence in a modal.
        if (data?.requiresStartSelection) {
          setStartNumberYear(Number(data.year) || new Date().getFullYear())
          setStartSequenceInput("1")
  ...
  const confirmStartSequence = async () => {
    const parsed = Number.parseInt(startSequenceInput.trim(), 10)
    if (!Number.isFinite(parsed) || parsed < 1 || parsed > 99999) {
      toast.error("Enter a valid starting number between 1 and 99999.")
      return
    }
    try {
      setLoadingBatchNumber(true)
      const data = await requestNextBatchNumber(parsed)
      setBatchNumber(data.nextBatchNumber)
      setShowStartNumberModal(false)
    } catch (error) {
      console.error('Error setting starting batch number:', error)
      toast.error("Failed to set starting batch number")
    } finally {
      setLoadingBatchNumber(false)
    }
  }
  ...
  return (
    <>
      <Dialog open={showStartNumberModal} onOpenChange={setShowStartNumberModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Choose Starting Batch Number</DialogTitle>
            <DialogDescription>
              No batches found for {startNumberYear}. Enter the starting sequence (1-99999).
              Example: <span className="font-mono">1</span> becomes{" "}
              <span className="font-mono">BCH-{startNumberYear}-00001</span>.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 pt-2">
            ...
            <Input
              ...
              min={1}
              max={99999}
              value={startSequenceInput}
              onChange={(e) => setStartSequenceInput(e.target.value)}
              placeholder="1"
            />
            <div className="flex justify-end gap-2 pt-1">
              <Button
                ...
                onClick={() => {
                  setStartSequenceInput("1")
                  void confirmStartSequence()
                }}
              >
                Use 00001
              </Button>
              <Button type="button" onClick={() => void confirmStartSequence()}>
                Confirm
              </Button>
            </div>
          </div>
