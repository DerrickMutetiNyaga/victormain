...
import { recordBulkPurgeOtpFailure } from '@/lib/jaba-bulk-purge-otp-failures'
import { acquireBulkBatchPurgeLock, releaseBulkBatchPurgeLock } from '@/lib/jaba-bulk-purge-lock'
import {
...
const BULK_PURGE_TOKEN_COLLECTION = 'jaba_bulk_batch_purge_tokens'
const BULK_DELETE_CONFIRM_PHRASE = 'DELETE ALL BATCHES'
...
/**
 * Super-admin only: delete every root batch (and each graph) after OTP + typed phrase + one-time purge token.
 * Not a single global Mongo transaction — each root purge is independent; a mid-run failure can leave partial state.
 */
export async function DELETE(request: Request) {
  ...
  const adminGate = await requireJabaSuperAdminDb({
    forbiddenMessage: 'Only super admins can delete all root batches',
  })
  if ('response' in adminGate) return adminGate.response
  const email = adminGate.email

  let body: { purgeToken?: string; confirmPhrase?: string }
  try {
    body = await request.json()
  } catch {
    return NextResponse.json({ error: 'JSON body with purgeToken and confirmPhrase is required' }, { status: 400 })
  }

  if (String(body?.confirmPhrase || '').trim() !== BULK_DELETE_CONFIRM_PHRASE) {
    return NextResponse.json(
      { error: `Confirmation phrase must be exactly: ${BULK_DELETE_CONFIRM_PHRASE}` },
      { status: 400 }
    )
  }

  const rl = checkRateLimit(`jaba-bulk-purge-exec:${email}`, 4, 24 * 60 * 60 * 1000)
  if (!rl.ok) {
    return NextResponse.json(
      { error: 'Too many bulk purge attempts. Try again later.', retryAfterMs: rl.retryAfterMs },
      { status: 429 }
    )
  }

  const purgeToken = String(body?.purgeToken || '').trim()
  if (!/^[a-f0-9]{56}$/i.test(purgeToken)) {
    return NextResponse.json({ error: 'Invalid purge token' }, { status: 400 })
  }
  ...
  const pre = await db.collection(BULK_PURGE_TOKEN_COLLECTION).findOne({
    tokenHash,
    requestedBy: email,
  })
  if (!pre) {
    await insertJabaSecurityAudit(db, {
      type: 'bulk_batch_purge_execute_failure',
      actorEmail: email,
      ip,
      userAgent: ua,
      meta: { reason: 'unknown_purge_token' },
    })
    return NextResponse.json({ error: 'Unknown or expired purge session. Run prepare again.' }, { status: 403 })
  }
  if (pre.invalidatedReason === 'otp_brute') {
    return NextResponse.json(
      {
        error: 'This purge session was locked after too many wrong OTP attempts. Prepare a new purge session.',
        code: 'OTP_BRUTE_LOCKED',
      },
      { status: 403 }
    )
  }
  if (pre.consumed === true) {
    return NextResponse.json(
      { error: 'This purge session was already used or voided. Prepare a new session.' },
      { status: 403 }
    )
  }
  const exp = pre.expiresAt instanceof Date ? pre.expiresAt : new Date(pre.expiresAt as string)
  if (exp <= new Date()) {
    return NextResponse.json({ error: 'Purge session expired. Prepare again.' }, { status: 403 })
  }

  const lock = await acquireBulkBatchPurgeLock(db, email)
  if (!lock.ok) {
    return NextResponse.json({ error: lock.error }, { status: 409 })
  }

    let purgedRoots = 0
    let batchDocsRemoved = 0
    const failures: string[] = []
    let lastFailureCode: string | undefined

    try {
      const otpCheck = await requireDeleteOtp(request, 'delete_all_batches', purgeToken)
    if ('response' in otpCheck) {
      if (otpCheck.otpInvalid) {
        await recordBulkPurgeOtpFailure(db, purgeToken)
      }
      return otpCheck.response
    }

    const claim = await db.collection(BULK_PURGE_TOKEN_COLLECTION).updateOne(
      {
        tokenHash,
        requestedBy: email,
        consumed: false,
    ...
    if (claim.modifiedCount !== 1) {
      return NextResponse.json(
        {
          error:
            'Purge session could not be claimed (race or expiry). Your OTP may have been used; prepare a new session if needed.',
        },
        { status: 403 }
      )
    }
    ...
    const roots = await db.collection('jaba_batches').find(rootQuery).sort({ createdAt: 1 }).toArray()
    const rootIds = roots.map((r) => r._id.toString())

    await insertJabaSecurityAudit(db, {
      type: 'bulk_batch_purge_execute_start',
      actorEmail: email,
      ip,
      userAgent: ua,
      meta: { rootBatchCount: rootIds.length, tokenHashPrefix: tokenHash.slice(0, 12) },
    })

    try {
      for (const rid of rootIds) {
        const res = await purgeJabaBatchGraphByRootId(db, rid)
        if (res.ok) {
          purgedRoots += 1
          batchDocsRemoved += res.deletedCount
          await insertJabaSecurityAudit(db, {
            type: 'bulk_batch_purge_root_ok',
            actorEmail: email,
            ip,
            userAgent: ua,
            meta: {
              ...
              batchDocumentsDeleted: res.deletedCount,
            },
          })
        } else if ('notFound' in res && res.notFound) {
          await insertJabaSecurityAudit(db, {
            type: 'bulk_batch_purge_root_skipped',
            actorEmail: email,
            ip,
            userAgent: ua,
            meta: { rootBatchId: rid, reason: 'not_found' },
          })
          continue
        } else {
          const errMsg = 'error' in res ? res.error : 'unknown'
          failures.push(`${rid}: ${errMsg}`)
          if ('code' in res && res.code) {
            lastFailureCode = res.code
          }
          await insertJabaSecurityAudit(db, {
            type: 'bulk_batch_purge_root_failed',
            actorEmail: email,
            ip,
            userAgent: ua,
            meta: {
      ...
      console.error('[Batches API] Bulk purge error:', purgeErr)
      await insertJabaSecurityAudit(db, {
        type: 'bulk_batch_purge_execute_failure',
        actorEmail: email,
        ip,
        ...
        meta: {
          exception: msg,
          purgedRootBatches: purgedRoots,
          batchDocumentsDeleted: batchDocsRemoved,
        },
      })
      return NextResponse.json(
        {
          success: false,
          error: 'Bulk purge failed partway',
          details: msg,
          purgedRootBatches: purgedRoots,
          batchDocumentsDeleted: batchDocsRemoved,
          warning:
            'Data may be partially inconsistent. Review batches, packaging outputs, delivery notes, and stock.',
        },
        { status: 500 }
      )
    }
    ...
    if (failures.length > 0) {
      console.error('[Batches API] Bulk purge failures:', failures)
      await insertJabaSecurityAudit(db, {
        type: 'bulk_batch_purge_execute_partial',
        actorEmail: email,
        ip,
        userAgent: ua,
        meta: {
      ...
      const isTxnEnv =
        lastFailureCode === JABA_PURGE_MONGODB_TRANSACTIONS_REQUIRED_CODE
      return NextResponse.json(
        {
          success: false,
          bulkSemantics: 'best_effort_across_roots',
          error: failures[0] || 'Bulk purge failed',
          ...(lastFailureCode ? { code: lastFailureCode } : {}),
          purgedRootBatches: purgedRoots,
          batchDocumentsDeleted: batchDocsRemoved,
          failures,
          warning: isTxnEnv
            ? 'MongoDB must support replica set transactions for purge. Earlier roots in this run may already have been purged; this failure does not roll them back.'
            : 'Purge stopped early; some batches may remain. Each completed root was committed in its own transaction. Review inventory and linked records before continuing production.',
        },
        { status: isTxnEnv ? 503 : 500 }
      )
    }

    await insertJabaSecurityAudit(db, {
      type: 'bulk_batch_purge_execute_success',
      actorEmail: email,
      ip,
      userAgent: ua,
      meta: { purgedRootBatches: purgedRoots, batchDocumentsDeleted: batchDocsRemoved },
    })
    ...
    return NextResponse.json({
      ...
      bulkSemantics: 'best_effort_across_roots',
      message:
        'All root batches and linked production data were removed; raw materials and packaging stock were restored where applicable (ingredients and packaging rows that record materialsUsed).',
      integrityNote:
        'Each root batch graph is purged in its own MongoDB transaction (all-or-nothing per root). This bulk action runs roots in order and is best-effort across roots: a failure after some roots succeed does not roll back earlier roots. Legacy packaging rows without materialsUsed are deleted without automatic raw stock refund. Avoid concurrent batch writes during purges.',
    })
  } finally {
    await releaseBulkBatchPurgeLock(db)
  }
}
