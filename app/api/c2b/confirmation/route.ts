...
export async function POST(request: Request) {
  try {
    ...
    if (accountReference) {
      // Narrow query to the exact order reference only — avoid matching by phone which can hit wrong orders
      const orderQuery = {
        $or: [
          { id: accountReference },
      ...
      // Only deduct stock if it has NOT been deducted yet (guard against duplicate C2B callbacks / POS-already-deducted)
      const items = order && !order.stockDeducted
        ? filterInventoryStockLineItems(order.items)
        : []
      const orderId = order?.id || accountReference
      ...
      if (order && items.length > 0) {
        ...
        if (!validation.ok) {
          ...
          if (deductOk) {
            const c2bOrd = await db.collection('orders').updateOne(
            ...
            if (c2bOrd.matchedCount === 0) {
              console.log('[M-Pesa C2B Confirmation] Order already finalized — skip duplicate C2B order update:', order.id)
            } else {
              console.log('[M-Pesa C2B Confirmation] Payment finalized on existing order only (C2B):', order.id)
            }
          }
        }
      } else if (order) {
        if (order.stockDeducted) {
          console.log(`[M-Pesa C2B Confirmation] Stock already deducted for order ${order.id} — skipping deduction`)
        }
        const c2bOrd2 = await db.collection('orders').updateOne(
          { _id: order._id, status: { $ne: 'completed' } },
          {
            $set: {
              paymentStatus: 'PAID',
        ...
        if (c2bOrd2.matchedCount === 0) {
          console.log('[M-Pesa C2B Confirmation] Order already finalized — skip duplicate C2B order update:', order.id)
        } else {
          console.log('[M-Pesa C2B Confirmation] Payment finalized on existing order only (C2B, no stock path):', order.id)
        }
      } else {
